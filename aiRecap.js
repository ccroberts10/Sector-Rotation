...
async function generateRecap(ctx) {
  const { rows, biotechSignal, shifts, regime } = ctx;

  // Compact data for the prompt
  const ranking = rows
    .map((r, i) => `${i + 1}. ${r.ticker} (comp ${r.composite.toFixed(1)}, 1m ${r.mo.toFixed(1)}%)`)
    .join('\n');
  ...
  const prompt = `You are an experienced market strategist writing a brief end-of-day note for an active trader.
...
1. What the rankings tell us about money flow right now
2. The single most actionable takeaway for a trader running a tech/semis-heavy momentum book
3. One thing to watch for tomorrow

Be specific. Reference actual sector tickers. Trader-to-trader tone, not corporate.`;
